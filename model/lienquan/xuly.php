...
$row = mysqli_fetch_assoc(mysqli_query($kun->connect_db(), "SELECT * FROM `nicklq` WHERE `id`='".$id."' AND `status`='true'"));
...
if($gift['id']) {
   ...
   $giatien =   $row['giatien']*$sale/100;
    ...
    $giatien =   $row['giatien'];
    $mota = 'Mua Nick Liên Quân #'.$row['id'].'';
}
...
if($giatien > $user['money']) {
    ...
    mysqli_query($kun->connect_db(), "INSERT INTO `user_history_system` (`username`, `action`, `action_name`, `sotien`, `mota`, `time`,`history`, `hisnick`) VALUES ('".$user['username']."', 'Mua Account', 'Mua Nick Liên Quân #".$row['id']."', '-".number_format($giatien)."đ', 'Mua Nick Liên Quân #".$row['id']."', '".time()."', '".$row['id']."','2')");
        // Update Tiền User
    mysqli_query($kun->connect_db(),"UPDATE `users` SET `money` = `money` - '".$giatien."' WHERE `username` = '".$user["username"]."'");
        // Update Trạng thái account
    mysqli_query($kun->connect_db(),"UPDATE `nicklq` SET `nguoimua` = '".$user['username']."', `status`='false' WHERE `id` = '".$id."'");
     die(json_encode(array('status' => 'success', 'msg' => 'Mua Nick Liên Quân #'.$row['id'].' Thành Công! Vui Lòng Kiểm Tra Lại Thông Tin Nick Trong Phần Tài Khoản Đã Mua!')));
