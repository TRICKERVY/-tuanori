...
$id = $_POST['id'];
$row = mysqli_fetch_assoc(mysqli_query($kun->connect_db(), "SELECT * FROM `random_lienquan_nick` WHERE `id`='".$id."' AND `status`='false' AND `nguoimua`='".$user['username']."'"));

if(!$row['id']) {
    die("<script>swal('Lỗi', 'Tài Khoản Này Không Tồn Tại!', 'error');</script>");
}else {
    ?>
<style type="text/css">
  .modal-dialog {
...
<div class="modal-header">
    ...
    <h4 class="modal-title">Thông Tin Tài Khoản Random #<?php echo $row['id'];?></h4>
</div>
<div class="modal-body">
        <div class="form-horizontal">
            ...
            <p class="c-font-bold c-font-blue">
                Tài khoản Liên Quân Random Mã Số #<?php echo $row['id'];?> mua lúc: <?php echo date('d/m/20y - H:i:s', $row['time']);?>
            </p>
                </center>            
