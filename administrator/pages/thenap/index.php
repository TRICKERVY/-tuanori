...
$start = isset($_REQUEST['page']) ? $page * $kmess - $kmess : (isset($_GET['start']) ? abs(intval($_GET['start'])) : 0);
 ...
 $sql = mysqli_query($hp->connect_db(), "SELECT * FROM `napthe` ORDER BY `time` DESC LIMIT $start, $kmess");
 $tong1 = mysqli_num_rows(mysqli_query($hp->connect_db(), "SELECT * FROM `napthe`"));
?>
...
if(isset($_POST['thatbai'])) {
  if($_POST['id']) {
    mysqli_query($hp->connect_db(), "UPDATE `napthe` SET `status`='0' WHERE `id`='".$_POST['id']."'");
...
if(isset($_POST['thanhcong'])) {
  if($_POST['id']) {
    $rose = mysqli_fetch_assoc(mysqli_query($hp->connect_db(), "SELECT * FROM `napthe` WHERE `id`='".$_POST['id']."'"));
    mysqli_query($hp->connect_db(),"UPDATE `users` SET `money` = `money` + '".$rose['amount']."' WHERE `username` = '".$rose["username"]."'");   
    mysqli_query($hp->connect_db(), "UPDATE `napthe` SET `status`='1' WHERE `id`='".$_POST['id']."'");
    echo '<div class="alert alert-success"><strong>PHÊ DUYỆT THÀNH CÔNG!</strong></div>';
    echo '<meta http-equiv=refresh content="1; URL=">';
  }
}
...
<?php
if ($tong1 > $kmess){
    echo '<center>' . $hp->phantrang('/administrator/thenap/', $start, $tong1, $kmess) . '</center>';
}
?>
