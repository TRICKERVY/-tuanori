...
$check = mysqli_num_rows(mysqli_query($kun->connect_db(), "SELECT COUNT(*) FROM `nicklq` WHERE `id`='".$id."'"));
...
$result = mysqli_query($kun->connect_db(), "SELECT * FROM `nicklq` WHERE `id` = '".$id."' LIMIT 1");
...
<img data-src="<?php echo $kun->get_thumb_lienquan($getdv['id']);?>" class="border border-gray-400 mb-2 w-full lazyLoad" src="<?php echo $kun->get_thumb_lienquan($getdv['id']);?>">
                 
   <?php 
        $path = $_SERVER["DOCUMENT_ROOT"]."/upload/nicklq/";
        $dir = $path.'info/'.$id;
                  ...
                  if ($opendirectory = opendir($dir)){
                    while (($file = readdir($opendirectory)) !== false){
                        ...
                        if ($file != "." && $file != "..") {
                        echo '<img data-src="/upload/nicklq/info/'.$id.'/'.$file.'" class="border border-gray-400 mb-2 w-full lazyLoad" src="/upload/nicklq/info/'.$id.'/'.$file.'">';
                }
                    }
